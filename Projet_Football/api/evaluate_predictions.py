...
def calculate_metrics_for_match(pred_home_prob, pred_draw_prob, pred_away_prob, actual_result):
    ...
    brier_score = sum((probs[outcome] - actual_one_hot[outcome]) ** 2 for outcome in ['H', 'D', 'A'])
...
def evaluate_recent_matches(days_back=7):
    ...
    fixtures_response = supabase.table("fixtures") \
    ...
    for pred in predictions:
        ...
        if ph is not None and pd_val is not None and pa is not None:
             ...
             if brier_1x2 is not None:
                 ...
                 existing_res = supabase.table('prediction_results') \
                     .select('id') \
                 ...
                 if existing_res.data:
                     # Update
                     supabase.table('prediction_results').update(record).eq('id', existing_res.data[0]['id']).execute()
                 else:
                     # Insert
                     supabase.table('prediction_results').insert(record).execute()
                     
                 updates_count += 1
                 print(f"Match {fix['id']} ({actual_result}) -> Brier: {brier_1x2:.3f}, LogLoss: {ll_1x2:.3f}")
