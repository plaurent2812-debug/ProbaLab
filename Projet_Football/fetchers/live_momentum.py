...
def save_match_cache(fixture_id, cache_data):
    ...
    result = supabase.table("football_momentum_cache").select("id").eq("api_fixture_id", fixture_id).execute()
    if result.data:
        supabase.table("football_momentum_cache").update({
            "stats_history": cache_data,
            "last_updated": now
...
def run_momentum_tracker():
    ...
    for fix in live_fixtures:
        ...
        if is_ht_window:
            ...
            def check_ht(stats):
                return stats["xg"] > 1.2 and stats["total_shots"] > 10 and stats["shots_on_target"] >= 4
        ...
        # ─── LOGIQUE 70ème MINUTE ───
            ...
            def check_70(stats):
                return stats["xg"] > 1.5 and stats["total_shots"] > 8 and stats["shots_on_target"] >= 3
    ...
    if alerts:
        ...
        for a in alerts:
            ...
            msg += f"📊 Stats de la période ciblée :\n"
